function Vmap( dom, mallId, floorId ) {
    ...
    var uaFlag = function() {
		...
		alert(ua);
        return ua.indexOf('qq') > -1 ? true : false ;  
    }();
    ...
    var rate = uaFlag ? 150*1000/25.4 : 96*1000/25.4;
    //当前地图的scale值
   ...
   // floorDiv.style.border = "5px solid red";
    //创建自己的canvas
    ...
    function drawFont( type, color, normalSize ) {
        ...
        for ( var i in publicPlace ) {
            ...
            if ( ( ( currentPublicType === "all" ) || ( currentPublicType === publicPlace[i].unit_type_eng ) ) && ( publicPlace[i].floor_id === currentFloorId ) ) {                context.fillText(publicPlace[i].font,publicPlace[i].coord_x*rate/currentScale*zoomScale-10,publicPlace[i].coord_y*rate/currentScale*zoomScale-10);
            }
        }
